...
public class UserPrefRepository {
    ...
    private String USER_TAG_KEY = "user_data"; // json으로 불러오기 위해 필요한 키 ( KEY, json) -> json의 내용 ( DataClass)이 KEY 값에 따라 불려온다.
    ...
    public void saveUserTags(ArrayList<VeganTag> tagList) {
        String json = gson.toJson(tagList);
        sharedPreferences.edit().putString(USER_TAG_KEY,  json).apply();
    }

    public ArrayList<VeganTag> loadTagList(List<VeganTag> tagList){
        String json = sharedPreferences.getString(USER_TAG_KEY, null);
        if( json != null ) {
            Type type = new TypeToken<ArrayList<VeganTag>>() {}.getType();
